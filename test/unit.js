...
describe('Unit', function() {
  ...
  it('has property accesors "GMC", "mBTC", "uBTC", "bits", and "satoshis"', function() {
    var unit = new Unit(1.2, 'GMC');
    unit.GMC.should.equal(1.2);
  ...
  it('a string amount is allowed', function() {
    ...
    unit.GMC.should.equal(0.12285714);
  });
  ...
  it('exposes unit codes', function() {
    should.exist(Unit.GMC);
    Unit.GMC.should.equal('GMC');
  ...
  it('exposes a method that converts to different units', function() {
    var unit = new Unit(1.3, 'GMC');
    unit.to(Unit.GMC).should.equal(unit.GMC);
  ...
  it('exposes shorthand conversion methods', function() {
    var unit = new Unit(1.3, 'GMC');
    unit.toBTC().should.equal(unit.GMC);
  ...
  it('can be imported and exported from/to JSON', function() {
    var json = JSON.stringify({amount:1.3, code:'GMC'});
  ...
  it('fails when the unit is not recognized', function() {
    ...
    expect(function() {
      return new Unit(100, 'GMC').to('USD');
    }).to.throw(errors.Unit.UnknownCode);
  });
  ...
  it('fails when the exchange rate is invalid', function() {
    ...
    expect(function() {
      return new Unit(100, 'GMC').atRate(-123);
    }).to.throw(errors.Unit.InvalidRate);
  });
